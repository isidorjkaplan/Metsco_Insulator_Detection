...
source_data_folder = "/home/isidor/Documents/keras/data/[FINAL] Sorted Insulators/4 Insulators"
batches = 1

#Cutoffs
no_insulator_cutoff = 0.001
crop_cutoff_value = 0.01
...
def get_crop_bounds(mask_arr):
    ...
    for row in range(0, img_height):
        if np.max(mask_arr[row]) > crop_cutoff_value:
            bottom = row
    for row in range(img_height - 1, 0, -1):
        if np.max(mask_arr[row]) > crop_cutoff_value:
            top = row
    for col in range(0, img_width):
        if np.max(mask_arr[:, col]) > crop_cutoff_value:
            right = col
...
for batch in range(0, batches):
    ...
    for i in range(0, len(label)):
        ...
        if avg > no_insulator_cutoff:
            ...
            mask_image = Image.fromarray((mask_image_arr * 255).astype(np.uint8))
            mask_image = mask_image.crop(box)
            mask_image.save(save_file_mask)

            frame_image = Image.fromarray(((crop_img[i]) * 255).astype(np.uint8))
            frame_image = frame_image.crop(box)
